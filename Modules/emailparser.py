...
class EmailParser:
#Default arguments 
	def __init__(self, filename, searchType):
		self.filename = filename
		self.lines = [None]*4
		self.date = None
		self.searchType = searchType
		#Below is a list of known timestamp formats. 
		self.formatlist = ["%d %b %Y %H:%M:%S %Z","%Y-%m-%dT%H:%M:%SZ","%Y-%m-%dT%H:%M:%S.%fZ","%Y-%m-%d %H:%M:%S.%f GMT"]
		self.stripList = ["\n","\r"]
	...
	#No return data, but adds info to the self.lines[] array, which is ultimately returned by the class and the doAll() function.
		if line.find("Timestamp") != -1 or line.find("First found") != -1:
			if line.find("Recent") == -1:
				...
				newline = self.timeFormat(newline, self.searchType)
...
#Test Case:	
#test = EmailParser('/home/adam/Documents/ElasticSearch/Automation/TestFiles/Email1.txt', 'BRO')
#print test.doAll()